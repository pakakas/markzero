...
2. **MarkZero**: The Protocol envelope used to switch between human and machine text. See [MarkZero Protocol Specification](./markzero-spec.md).
...
| **Row Separator** | `¦` | U+00A6 (BROKEN BAR) | Separates cells in a Grid row. Trailing `¦` for empty last cells may be omitted — decoder infers missing cells as `""`. |
...
- **Multi-Grid Capability**: A single ADN payload can contain **multiple** Grid blocks (`ⓖ`), each with an optional title embedded between `ⓖ` and `ᴄ`, allowing complex multi-set or multi-grid structures to be serialized and parsed in a single stream.
...
- **Empty Collections**: An empty Map `{}` and an empty List/Set `[]` both serialize to `ⓖ` (Grid Start) to maintain structural minimalism.
- **Trailing Separator Omission**: Encoders MAY strip trailing `¦` separators when the last cells in a row are empty. Decoders MUST treat missing trailing cells as empty strings (`""`). This saves one token per omitted cell.
...
*   **Literal Marker**: To include a structural marker (like `¦`, `ⓖ`, `ᴄ`, or `ʀ`) as literal data, prefix it: `ɛ¦`, `ɛⓖ`, `ɛᴄ`, `ɛʀ`.
